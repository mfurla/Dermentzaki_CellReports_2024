...
### Sites probabilities
KO_site_proba <- read.table("m6Anet_KO_data.site_proba.csv",sep=",",header=TRUE)
WT_site_proba <- read.table("m6Anet_WT_data.site_proba.csv",sep=",",header=TRUE)

### Methylated sites
sum(KO_site_proba[,"probability_modified"]>0.9)/nrow(KO_site_proba)*100 # 0.3344113
sum(WT_site_proba[,"probability_modified"]>0.9)/nrow(WT_site_proba)*100 # 2.498397

pdf("fractionOfMethylatedSites0.9.pdf",width=2,height=4)
barplot(c("WT1"=sum(WT_site_proba[,"probability_modified"]>0.9)/nrow(WT_site_proba)*100
		 ,"KD1"=sum(KO_site_proba[,"probability_modified"]>0.9)/nrow(KO_site_proba)*100)
,ylab="[%]",main="Fraction of\nmethylated sites",las=2)
dev.off()